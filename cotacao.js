...
function main(){
    let currency = { "BRL": "Real, "EUR: "Euro",
    "USD": "Dollar"};
    let options = [];
...
async function calculate(){
    ...
    let { rates } = await
getURL(`https://api.exchangerate-api.com/v4/latest/${from}`);
let rate = rates[to];
tax_info.innerText = `1
${getInfoSelect(label_from_currency)} = ${rate}
${getINfoSelect(label_to_currency)}`
